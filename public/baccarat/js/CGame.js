function CGame(oData){
    ...
    this.reset = function(bFirstPlay){
        ...
        _oInterface.reset();
        _oInterface.enableBetFiches(bFirstPlay);
    ...
    this.changeState = function(iState){
        ...
        if(_iState === STATE_GAME_DEALING){
            ...
            if(iRand < WIN_OCCURRENCE){
                ...
                var iWinningBet;
                do{
                    var iRandBet = Math.floor(Math.random()*100);
                    if(iRandBet < BET_OCCURRENCE[WIN_TIE]){
                        iWinningBet = WIN_TIE;
                    }else if(iRandBet < BET_OCCURRENCE[WIN_DEALER]){
                        iWinningBet = WIN_DEALER;
                    }else{
                        iWinningBet = WIN_PLAYER;
                    }
    ...
    this.setBet = function(iFicheValue,iFicheIndex,iTypeBet){
        ...
        if( (iTotBet+iFicheValue) <= _iMaxBet && iFicheValue <= _oSeat.getCredit() ){
            iTotBet+=iFicheValue;
            iTotBet=Number(iTotBet.toFixed(1));
